  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 nav-blur
        ${scrolled ? 'bg-[#020B18]/90 border-b border-[#c0257d]/20 py-3' : 'py-5 sm:py-6'}`}
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-6 lg:gap-8">
          {links.map((link) => (
            <li key={link.href}>
              ...
              hover:text-[#e040a0] transition-colors duration-300"
              >
                {link.label}
              </a>
            </li>
        ...
        <a
          href="#rsvp"
          className="hidden md:block px-4 lg:px-5 py-2 border border-[#c0257d]/50 text-[#e040a0]
      font-sans text-xs tracking-widest uppercase hover:bg-[#c0257d]/10
      hover:border-[#e040a0] transition-all duration-300 shrink-0"
        >
          RSVP Now
        </a>
        ...
        <button
          ...
          aria-label="Menu"
        >
          <span className={`block w-6 h-px bg-[#e040a0] transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2.5' : ''}`} />
          <span className={`block w-6 h-px bg-[#e040a0] transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-6 h-px bg-[#e040a0] transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2.5' : ''}`} />
        </button>
      </div >
      ...
      {
        menuOpen && (
          <div className="md:hidden bg-[#040f1e]/98 border-t border-[#c0257d]/20 py-6">
            <ul className="flex flex-col items-center gap-5">
              {links.map((link) => (
                <li key={link.href}>
                  <a
                    ...
                    hover:text-[#e040a0] transition-colors"
                  >
                    {link.label}
                  </a>
                </li>
              ...
              <li className="mt-2">
                <a href="#rsvp" onClick={() => setMenuOpen(false)}
                  className="px-8 py-3 border border-[#c0257d]/50 text-[#e040a0]
                  font-sans text-xs tracking-widest uppercase hover:bg-[#c0257d]/10 transition-all duration-300">
                  RSVP Now
                </a>
              </li>
            </ul>
